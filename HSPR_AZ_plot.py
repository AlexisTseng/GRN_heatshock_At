...
--saveAlsoSvg,-svg
    Whether to save the figure in both svg and pdf format. Default is only pdf. (default: 1)
...
def main(opt):
    ...
    param_dict = param_extract(data_dir, opt)
    ...
    print("Step4: Plot Simulation Outcome")
    ...
    plot_outcome(data_df, plot_dir, param_dict, opt)
...
#######################################################################
def plot_outcome(data_df, plot_dir, param_dict, opt):
    ### number of rows and columns for all iterations
    Rows = int(math.sqrt(param_dict['numberofiteration']))
    Columns = int(math.ceil(param_dict['numberofiteration'] / Rows))
